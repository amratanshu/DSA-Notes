https://www.youtube.com/watch?v=V6H1qAeB-l4&list=PLgUwDviBIf0oE3gA41TKO2H5bHpPd7fzn&index=33&ab_channel=takeUforward

We can do this using priority queue or Set

Using PQ ->

Put pair<(distance, node)> on the node 
So the top ele will always be sorted on DIST followed by NODE

Maintain a distance array for each node

Start doing sort of a BFS traversal. First put the starting node and distance 0 on the PQ

In the bfs - only push an adjacent node if the top.dist + adjNode.dist < dist[adjNode]
